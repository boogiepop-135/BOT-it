...
export const run = async (message: Message, args: string[] = null, userI18n: UserI18n) => {
    ...
    if (!city) {
        message.reply(`> Levi Assistant 💻 Por favor especifica una ciudad, por ejemplo: ${AppConfig.instance.getBotPrefix()}meteo Ciudad de México.`);
        return;
    }
    ...
    if (city) {
        try {
            ...
            logger.error(error);
            message.reply('> Levi Assistant 💻 El servicio del clima no está disponible actualmente. Por favor intenta más tarde.');
        }
    }
};
