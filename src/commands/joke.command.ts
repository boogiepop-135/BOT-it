...
export const run = async (message: Message, _args: string[] = null, userI18n: UserI18n) => {
    ...
    if (!jokeData) {
        await message.reply(
            ...
            null,
            { sendVideoAsGif: true, caption: "> Levi Assistant 💻 No se encontró chiste" },
        );
        return;
    }
    ...
    if (jokeData.type === "twopart") {
        ...
        const setupJoke = await message.reply(
            media,
            null,
            { sendVideoAsGif: true, caption: `> Levi Assistant 💻 1/2 :\n${jokeData.setup}\n\n...` },
        );

        if (jokeData.delivery) {
            setTimeout(async () => {
                await setupJoke.reply(AppConfig.instance.printMessage(`> Levi Assistant 💻 2/2 :\n${jokeData.delivery}\n😂😂🤣`));
            }, 5000);
        }
        ...
        await message.reply(
            MessageMedia.fromFilePath(AppConfig.instance.getBotAvatar("confused")),
            null,
            { sendVideoAsGif: true, caption: "> Levi Assistant 💻 Error al obtener chistes" },
        );
        return;
    }
};
